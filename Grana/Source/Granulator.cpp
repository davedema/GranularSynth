...
void Granulator::initialize()
{
    ...
    AudioBuffer<float>* shiftedBuffer = lastActivatedGrain->freqShift(1);
    AudioBuffer<float>* nextShiftedBuffer = nextActivatedGrain->freqShift(1);
...
void Granulator::process(AudioBuffer<float>& outputBuffer, int numSamples)
{
    ...
    for (int samplePos = 0; samplePos < numSamples; samplePos++) {
        ...
        if (currentSampleIdx == lastInterOnset + this->totalHops) {
            ...
            AudioBuffer<float>* shiftedBuffer = lastActivatedGrain->freqShift(1), *previousBuffer;
