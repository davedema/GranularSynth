...
*
* Purpose:
*   Run a set of timing tests on the above functions.
************************************************************/
...
/*
 * Each test is run for a number of iterations estimated
 * to take approximately T_RUN seconds.  A test is run
 * for T_ESTIMATE seconds to compute the estimate, then the
 * actual test is run without clock-checking overhead.
 */
#define T_ESTIMATE 0.1  /* [s] */
#define T_RUN 0.5       /* [s] */

struct ft_bmark {
    const char* name; /* descriptive name of test */
    void (*func)(double*, unsigned long); /* function to test */
    size_t mem_size; /* total memory footprint */
    size_t obj_size; /* i.e. double or complex */
} ft_bmarks[] = {
    {"", NULL, 0, 0},
